...
export default function LifoCalculator() {
  ...
  const { history, saveEntry, clearHistory, deleteEntry } = useCalcHistory<{ batch1Units: string; batch1Cost: string; batch2Units: string; batch2Cost: string; unitsSold: string }>('lifo-calculator');
  ...
  const fromB2 = Math.min(sold, b2u);
  const fromB1 = Math.max(0, Math.min(sold - b2u, b1u));
  const cogs = fromB2 * b2c + fromB1 * b1c;
  ...
  const handleCalculate = () => {
    if (sold > 0 && b1u > 0) {
      setCalculated(true);
      saveEntry({ batch1Units, batch1Cost, batch2Units, batch2Cost, unitsSold }, `LIFO COGS: ${formatCurrency(cogs)} (ending inv: ${formatCurrency(endingInventory)})`);
    }
  };
  ...
  const snap = (): Snapshot => ({ b1u, b1c, b2u, b2c, sold, fromB1, fromB2, cogs, remainingB1, remainingB2, endingInventory, label: `LIFO COGS: ${formatCurrency(cogs)}` });
    ...
    <div className="w-full">
      <div className="p-4 sm:p-6 space-y-6">
        <div className="rounded-lg bg-emerald-500/5 border border-emerald-500/20 p-3 text-xs text-muted-foreground">
          ...
          {`Calculate COGS and ending inventory using LIFO (last in, first out) method.`}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="batch2Units">Batch 2 Units (purchased last)</Label>
            <div className="relative">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="result-display mt-2" aria-live="polite">
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6 space-y-6">
              <div className="text-center space-y-2">
                <p className="text-sm text-muted-foreground font-medium">LIFO COGS</p>
                <p className="text-4xl font-bold text-emerald-600">{formatCurrency(cogs)}</p>
                <div className="flex justify-center gap-2 flex-wrap">
                  <Badge variant="outline" className="'bg-emerald-500/10 border-emerald-500/30 text-emerald-600'">{`From batch 2: ${fromB2} units @ ${formatCurrency(b2c)}`}</Badge>
                  <Badge variant="outline" className="'bg-blue-500/10 border-blue-500/30 text-blue-600'">{`From batch 1: ${fromB1} units @ ${formatCurrency(b1c)}`}</Badge>
              ...
              <div className="rounded-lg bg-muted/30 border border-border/30 p-3 text-xs text-muted-foreground">
                {`LIFO (Last In First Out): newest inventory is sold first. COGS uses latest purchase costs. Ending inventory uses earliest costs. In rising prices, LIFO gives higher COGS, lower profit, lower ending inventory. Not allowed under IFRS.`}
              </div>
            </div>
          </motion.div>
        )}
